...
const faqs = [
  ...
  { q: 'How much does EFTPOS cost a plastering business per month?', a: 'A plastering business doing $10,000/month in renovation card payments pays $140/month with Zeller (1.4%) — or uses Tap to Pay for free (phone only). Builder sub-contract invoicing via Stripe costs 1.7% + $0.10 per card transaction, or 0.8% for bank transfer. For high-value builder invoices, always offer bank transfer to reduce fees.' },
  { q: 'Is it hard to set up EFTPOS compared to a bank?', a: 'No — Zeller and Stripe both approve accounts online with just your ABN. Bank merchant accounts require paperwork and often take days or weeks. For renovation work, start with Zeller Tap to Pay (no hardware cost) and add a terminal when your volume grows.' },
  { q: 'Can plasterers track payments across multiple jobs at once?', a: 'Yes — Stripe invoicing assigns a unique invoice number to each job and tracks payment status (outstanding, paid, overdue) in a dashboard. For multiple sites running concurrently, Stripe is far easier to reconcile than a stack of terminal receipts. Xero and MYOB both have direct Stripe integrations for automatic bookkeeping.' },
]

import siteMeta from '../../data/site-meta.json'
const SITE = 'https://tradiepayau.directory'
const jsonLd = [
  { '@context': 'https://schema.org', '@type': 'Article', headline: 'Best EFTPOS for Plasterers in Australia (2026)', name: 'Best EFTPOS for Plasterers in Australia (2026)', image: { '@type': 'ImageObject', url: 'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?w=1200&h=630&fit=crop&crop=center&q=80', width: 1200, height: 630 }, description: 'Builder sub-contracting, dusty environments, staged billing, and same-day material cash flow — the best EFTPOS and payment setup for Australian plasterers.', url: `${SITE}/trades/plasterers`, datePublished: '2026-01-15', dateModified: siteMeta.lastVerified, author: { '@type': 'Organization', name: 'TradiePay AU', url: SITE }, publisher: { '@type': 'Organization', name: 'TradiePay AU', url: SITE } },
  ...
  { '@context': 'https://schema.org', '@type': 'Service', name: 'Best EFTPOS for Plasterers in Australia', serviceType: 'EFTPOS Payment Processing for Plasterers', areaServed: { '@type': 'Country', name: 'Australia' }, provider: { '@type': 'Organization', name: 'TradiePay AU', url: SITE }, url: `${SITE}/trades/plasterers` },
]
    ...
    <>
      ...
      <header className="hero relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <img src="https://images.unsplash.com/photo-1541888946425-d81bb19240f5?w=900&h=560&fit=crop&crop=center&q=80" alt="" fetchPriority="high" className="w-full h-full object-cover" onError={e => { e.currentTarget.style.opacity = '0' }} />
      ...
      <section className="section section-alt">
        <div className="container-page">
          <h2 className="text-xl font-bold text-brand-dark mb-4">Best EFTPOS for Plasterers by State</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {STATES.map(s => (
              <Link
                key={s.slug}
                to={`/states/${s.slug}/plasterers`}
