...
const PAIN_POINTS = [
  {
    emoji: '🏗️',
    title: 'Job sites with no building infrastructure yet',
    body: 'Slab pours, driveways, and footings happen before the building exists — no power, no NBN, no WiFi. You need to take payment on-site the moment the pour is done. The right terminal runs on its own SIM.',
  },
  {
    emoji: '🌏',
    title: 'Rural and acreage pours — poor signal, long drives',
    body: 'Country driveways, farm shed slabs, and rural housing pads are often beyond reliable WiFi or phone hotspot range. Zeller\'s built-in Optus SIM handles most sites. Square\'s offline mode is the backup for genuine zero-signal locations.',
  },
  {
    emoji: '💸',
    title: 'Large deposits — concrete + pump + labour locked in early',
    body: 'Concrete trucks, pump hire, and labour are all committed before pour day. Clients need to lock in a deposit at quote acceptance to cover the order. Zeller payment link for the deposit. Terminal for the balance on the day.',
  },
  {
    emoji: '☀️',
    title: 'Weather-dependent completion — payment confirmed same day',
    body: 'Pour day is locked in by the weather. If you can\'t get payment signed off on the day, you\'re chasing it across a wet week with a client who has already moved on. Same-day terminal means same-day receipt.',
  },
]

const COMPARISON = [
  { provider: 'Zeller Terminal 1 + SIM', rate: '1.4%', cost: '$99 + $15/mo', note: 'Primary — job sites, rural pours',          recommended: true },
  { provider: 'Square Terminal',         rate: '1.6%', cost: '$329 once',     note: 'Backup — remote areas, offline mode',      recommended: false },
  { provider: 'Zeller Payment Link',     rate: '1.4%', cost: 'Free',          note: 'Deposits before pour day',                 recommended: false },
]

const STEPS = [
  { n: '1', title: 'Sign up with your ABN',          body: 'Online approval in minutes — just your ABN. No bank branch visit, no paperwork.' },
  { n: '2', title: 'Hardware ships fast',             body: 'Zeller Terminal ships within Australia. Most concreters receive it in 1–3 business days.' },
...
export default function ConcretersLPBase({ config }) {
    ...
    <>
      ...
      <header className="hero relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <img src={tradeHeroUrl('concreters')} alt={tradeHeroAlt('concreters')} fetchPriority="high" className="w-full h-full object-cover" onError={e => { e.currentTarget.style.opacity = '0' }} />
      ...
      <section className="section-alt py-10 sm:py-14">
        <div className="container-page">
          <div className="mb-8 pb-8 border-b border-slate-200">
            <h2 className="text-xl font-bold text-brand-dark mb-2">Best EFTPOS for concreters in Australia (2026)</h2>
            <p className="text-sm text-slate-600 leading-relaxed max-w-2xl">For Australian concreters, choosing the right EFTPOS matters — especially for bare construction sites, rural pours, and weather-dependent jobs. This guide compares Zeller, Square, Stripe, and other options based on real concreting use cases, including rates, SIM connectivity, and same-day settlement.</p>
      ...
      <section className="section bg-white">
        <div className="container-page">
          <div className="max-w-2xl">
            <p className="text-[11px] font-bold uppercase tracking-widest text-brand-blue mb-2">Top pick for concreters in Australia</p>
            <h2 className="text-2xl font-bold text-brand-dark mb-1">Zeller Terminal 1 + SIM Plan</h2>
            <p className="text-slate-500 text-sm mb-6 leading-relaxed">The go-to EFTPOS setup for Australian concreters on residential pours, commercial slabs, and rural properties. Runs on the Optus mobile network — no site WiFi needed.</p>
      ...
      <section className="section section-alt">
        <div className="container-page">
          ...
          <p className="text-sm text-slate-500 mb-5">The three setups used by Australian concreters — when and why.</p>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 max-w-lg">
            ...
            <div className="space-y-3">
              {[
                ...
                { q: 'Does it work on bare construction sites?',        a: 'The Zeller SIM handles most construction sites without building WiFi or power. Add Square Terminal as a backup for rural or remote locations with zero connectivity.' },
                { q: 'What about rural properties and farm slabs?',     a: 'Zeller runs on the Optus mobile network, which covers most of regional Australia. For very remote sites, Square\'s offline mode accepts cards with no connectivity at all.' },
                { q: 'Can I take a deposit before the pour day?',       a: 'Yes — send a Zeller payment link at quote acceptance. The client pays remotely; you get the deposit cleared before committing to the concrete order.' },
              ].map(({ q, a }) => (
                <div key={q}>
      ...
      <section className="section">
        <div className="container-page">
          <div className="bg-brand-dark rounded-3xl px-6 py-10 sm:px-10 sm:py-12 text-center">
            <p className="text-amber-400 text-xs font-bold uppercase tracking-widest mb-3">For Australian concreters</p>
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3 leading-tight">Stop chasing invoices.<br className="hidden sm:block" /> Get paid on pour day.</h2>
